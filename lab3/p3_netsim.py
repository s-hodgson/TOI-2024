...
#	self.LSA.clear()
    ...
    def draw(self,dc,transform):
        ...
        dc.DrawRectangle(int(loc[0]-self.nsize), int(loc[1]-self.nsize),
                         int(2*self.nsize+1), int(2*self.nsize+1))
        label = str(self.address)
        dc.SetTextForeground('light grey')
        dc.SetFont(wx.Font(max(4, int(self.nsize*2)), wx.SWISS, wx.NORMAL, wx.NORMAL))
        dc.DrawText(label, int(loc[0]+self.nsize+2), int(loc[1]+self.nsize+2))

        if len(self.transmit_queue) > 0:
            self.transmit_queue[0].draw(dc,transform, int(loc[0]-2*self.nsize),int(loc[1]-2*self.nsize))
...
class Link:
    ...
    def draw(self,dc,transform):
        ...
        dc.DrawLine(int(n1[0]), int(n1[1]), int(n2[0]), int(n2[1]))
        # show link's cost near it
        dc.SetTextForeground('light grey')
        dc.SetFont(wx.Font(max(4, int(self.nsize*2)), wx.SWISS, wx.NORMAL, wx.NORMAL))
        dc.DrawText(self.costrepr, int((n1[0]+n2[0])/2), int((n1[1]+n2[1])/2))

        if self.broken:
            dc.SetPen(wx.Pen('red',3,wx.SOLID))
            midx = int((n1[0]+n2[0])/2)
            midy = int((n1[1]+n2[1])/2)
            offset = 0.1 * transform[0]
            dc.DrawLine(int(midx-offset), int(midy-offset), int(midx+offset), int(midy+offset))
            dc.DrawLine(int(midx+offset), int(midy-offset), int(midx-offset), int(midy+offset))
...
class Packet:
    ...
    def draw(self, dc, transform, px, py):
        ...
        dc.DrawCircle(int(px), int(py), int(radius))
