...
class WirelessNode:
    ...
    def draw(self, dc, transform):
        ...
        dc.DrawRectangle(loc[0]-self.nsize, loc[1]-self.nsize, 2*self.nsize+1, 2*self.nsize+1)
...
#        dc.SetFont(wx.Font(max(4, 2*self.nsize), wx.SWISS, wx.NORMAL, wx.NORMAL))
        dc.SetFont(wx.Font(max(4, int(1.5*self.nsize)), wx.SWISS, wx.NORMAL, wx.NORMAL))
...
class NetFrame(wx.Frame):
    def __init__(self, parent=None, id=-1, size=(1000, 500),
                 pos=wx.DefaultPosition, title='WSim', style=wx.DEFAULT_FRAME_STYLE):
